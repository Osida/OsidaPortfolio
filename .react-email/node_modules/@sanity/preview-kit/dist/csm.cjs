...
var sourcemap = require('./_chunks/sourcemap-52dc8a50.cjs');
...
function createTranscoder(config) {
  ...
  return (result, csm) => {
    ...
    return {
      result: sourcemap.encode(result, csm, (value, sourceDocument, path) => transcode(value, sourceDocument, path)),
      report
    };
  };
}
...
function mapToEditLinks(result, csm, studioUrl) {
  const createEditLink = defineEditLink(studioUrl);
  return sourcemap.encodeIntoResult(result, csm, (_, sourceDocument, path) => {
